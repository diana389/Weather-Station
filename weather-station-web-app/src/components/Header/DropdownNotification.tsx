...
import database from "@/app/firebaseConfig"; // Adjust the import according to your Firebase config
...
const DropdownNotification = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const notificationsRef = ref(database, "error/notifications"); // Adjust the reference path as needed

        console.log("Fetching notifications...");

        // Query to get all notifications (limit not applied yet)
        ...
        if (snapshot.exists()) {
          ...
          const titlesSet = new Set<string>(); // Set to store unique titles

          // Convert the data object into an array of notifications
          for (let key in data) {
            if (data[key]) {
              const notificationTitle = data[key]; // Assuming data has a `title` field

              // Skip if the title is already added
              if (titlesSet.has(notificationTitle)) {
                continue;
              }

              // Add the title to the set
              titlesSet.add(notificationTitle);

              // Create the notification object
              const notification: Notification = {
                image: "/images/user/alarm.jpg", // Assuming all notifications have this image
          ...
          // Limit to the last 5 unique notifications
          const last5Notifications = notificationsArray.slice(-5);

          setNotifications(last5Notifications); // Set the final filtered list of 5 unique notifications
          setNotifying(last5Notifications.length > 0); // Set the notifying state based on the filtered data
        } else {
      ...
      <li>
        ...
        >
          <span className="relative">
            ...
            >
              <span
                className="absolute -z-1 inline-flex h-full w-full animate-ping rounded-full bg-red-light opacity-75"></span>
            </span>
          </span>
        </Link>
