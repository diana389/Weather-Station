...
const Dashboard: React.FC = () => {
    ...
    const [user, setUser] = useState<any>(null); // This will store the authenticated user
    ...
    const splitIntoIntervals = (
        data: number[],
    ): { ranges: string[]; counts: number[] } => {
    ...
    // UseEffect to fetch last notification, regardless of login status
    useEffect(() => {
        if (user) {
            fetchLastNotification(); // Fetch notifications only if the user is logged in
        }
    }, [user]);
    ...
    const computeErrorMessage = (
        ...
        if (errorMessage) {
            ...
            let lastTimestamp =
                temperaturePairs[temperaturePairs.length - 1]?.timestamp;
    ...
    useEffect(() => {
        const temperatureRef = query(
            ref(database, "sensor_data/temperature"),
            limitToLast(100),
        );
        const humidityRef = query(
            ref(database, "sensor_data/humidity"),
            limitToLast(100),
        );
        const pressureRef = query(
            ref(database, "sensor_data/pressure"),
            limitToLast(100),
        );
        ...
        const processData = (
            ...
            if (snapshot.exists()) {
                ...
                const formattedData: SensorData[] = Object.entries(rawData)
                    .map(([entryKey, value]) => {
                        ...
                        console.warn(
                            `[${type}] Invalid entry format for key: ${entryKey}, value:`,
                            value,
                        );
    ...
    if (!user) {
        return <p>Please log in to view the dashboard.</p>; // Or your login component
    }

    if (loading) return <p>Loading...</p>;

    const latestTemperature = parseFloat(
        temperatureData[temperatureData.length - 1]?.toFixed(2) as string,
    );
    const latestHumidity = parseFloat(
        humidityData[humidityData.length - 1]?.toFixed(2) as string,
    );
    const latestPressure = parseFloat(
        pressureData[pressureData.length - 1]?.toFixed(2) as string,
    );
    ...
    const growthRateTemperature = calculateGrowthRate(
        temperatureData,
        averageTemperature,
    )?.toFixed(2);
    const growthRateHumidity = calculateGrowthRate(
        humidityData,
        averageHumidity,
    )?.toFixed(2);
    const growthRatePressure = calculateGrowthRate(
        pressureData,
        averagePressure,
    )?.toFixed(2);
    ...
    return (
        <>
            {errorMessage && (
                <div className="mb-8">
                    <AlertError title="Out of bounds!" message={errorMessage}/>
                </div>
            )}
            ...
            <div className="mb-6 mt-4 grid gap-4 md:mt-6 md:gap-6 2xl:mt-9 2xl:gap-7.5">
                <div className="flex w-full flex-wrap items-center gap-5">
                    ...
                    <div className="flex w-1/4 flex-shrink-0 justify-center">
                        <ButtonDefault
                            label="Trigger Alarm"
